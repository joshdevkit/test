...
<script>
    $(document).ready(function() {
            ...
            $('#btn_print_requisition_report').on('click', function() {
                ...
                fetch("{{ route('print-requisition-report') }}", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "X-CSRF-TOKEN": "{{ csrf_token() }}",
            ...
            $('#btn_print_requisition_all').on('click', function () {
                ...
                setTimeout(function () {
                    ...
                    const originalTable = document.getElementById('requisition_table');
                    ...
                    // Send to server
                    fetch("{{ route('print-requisition-report-all') }}", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                            "X-CSRF-TOKEN": "{{ csrf_token() }}",
                        },
                        body: JSON.stringify({ data: rows, title: 'REQUISITION RECORDS' }),
                    })
                    .then(response => response.blob())
                    .then(blob => {
